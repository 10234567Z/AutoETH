...
const AgentsPage = () => {
    ...
    useEffect(() => {
        ...
        if (anyWindow.ethereum) {
            anyWindow.ethereum.request({ method: 'eth_accounts' }).then((accounts: string[]) => {
    ...
    const fetchAgentsByWallet = async (addr: string) => {
        ...
        try {
            ...
            const provider = anyWindow.ethereum
                ? new (ethers as any).providers.Web3Provider(anyWindow.ethereum)
                : ethers.getDefaultProvider();

            const contract = new (ethers as any).Contract(CONTRACT_ADDRESS, CONTRACT_ABI, provider);
            ...
            for (const agentAddr of leaderboard) {
                try {
                    ...
                    const agentWalletAddr = a[1] as string;
                    if (agentWalletAddr && agentWalletAddr.toLowerCase() === addr.toLowerCase()) {
                        const totalGuesses = Number(a[2] || 0);
                        const bestGuesses = Number(a[3] || 0);
                        const accuracyPct = totalGuesses > 0 ? ((bestGuesses * 100) / totalGuesses).toFixed(1) + '%' : '0%';

                        results.push({
                            id: agentAddr,
                            name: agentAddr, // no human name on-chain; show agent address. You can map to Agentverse later.
                            avatar: `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(agentAddr)}`,
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex justify-between items-center mb-12">
                    ...
                    <div className="flex items-center gap-4">
                        {!walletConnected ? (
                            <button onClick={connectWallet} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-md font-semibold transition-all">Connect Wallet</button>
                        ) : (
                            <div className="text-sm text-gray-300">Connected: <span className="font-mono">{walletAddress}</span></div>
                        )}
                        <button onClick={() => setIsModalOpen(true)} className="bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-md font-semibold transition-all flex items-center gap-2">
                ...
                {loading && <div className="text-gray-400 mb-4">Loading agents on-chain...</div>}
                {error && <div className="text-red-400 mb-4">{error}</div>}

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {agents.length === 0 && !loading ? (
                        <div className="text-gray-400">No on-chain agents found for this wallet.</div>
                    ) : (
                        agents.map(agent => (
